...
module_energy_L114.solar <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "common/iso_GCAM_regID",
             FILE = "energy/Zhou_solar_supply_ctry_EJ"))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c("L114.RsrcCurves_EJ_R_solar"))
    ...
    Zhou_solar_supply_ctry_EJ <- get_data(all_data, "energy/Zhou_solar_supply_ctry_EJ")
    ...
    L114.RsrcCurves_EJ_R_solar <- lapply(unique(regional_price_supply_points$GCAM_region_ID), function(region_ID) {
      ...
      L114.RsrcCurves_EJ_R_solar.currR <- tibble(GCAM_region_ID = region_ID, base.price = min(L114.SupplyPoints.currR$price),
      ...
      half_max_resource = 0.5 * L114.RsrcCurves_EJ_R_solar.currR$maxSubResource
      ...
      L114.RsrcCurves_EJ_R_solar.currR %>%
      ...
      L114.error_min_curve.exp <- optimize(f = smooth_res_curve_approx_error, interval = c(1.0, 15.0),
                                           L114.RsrcCurves_EJ_R_solar.currR$mid.price,
                                           L114.RsrcCurves_EJ_R_solar.currR$base.price,
                                           L114.RsrcCurves_EJ_R_solar.currR$maxSubResource,
                                           L114.SupplyPoints.currR)

      # Add curve exponent to table
      L114.RsrcCurves_EJ_R_solar.currR$curve.exponent <- L114.error_min_curve.exp$minimum
  ...
  L114.RsrcCurves_EJ_R_solar %>%
    ...
    mutate(resource = "global solar resource",
           subresource = "global solar resource") %>%
    # Reorder columns
    select(GCAM_region_ID, resource, subresource, maxSubResource, mid.price, curve.exponent, base.price) %>%

  # ===================================================

    add_title("Solar resource curves by GCAM region") %>%
    ...
    add_legacy_name("L114.RsrcCurves_EJ_R_solar") %>%
    add_precursors("common/iso_GCAM_regID", "energy/Zhou_solar_supply_ctry_EJ") ->
    L114.RsrcCurves_EJ_R_solar

  return_data(L114.RsrcCurves_EJ_R_solar)
} else {
